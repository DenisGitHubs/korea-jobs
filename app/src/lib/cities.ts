...
/**
 * Human summary of the current city selection for the "Cities" nav rows
 * (filter sheet + settings). `noCity` = the "city not specified" toggle.
 *   []            + noCity on  -> "All cities"
 *   []            + noCity off -> "Without ‘not specified’"
 *   [one]                      -> its (disambiguated) name
 *   [many]                     -> "N cities"
...
export function citiesSummary(
  ...
  if (slugs.length === 1) {
    ...
    base = c ? cityLabel(c, lang, t) : t('filter.citiesCount', { count: 1 });
  } else {
    base = t('filter.citiesCount', { count: slugs.length });
  }
